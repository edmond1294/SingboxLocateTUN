...
install_basic() {
    ...
    if command -v apt-get >/dev/null 2>&1; then
        ...
        apt-get install -y curl wget ca-certificates unzip iproute2 python3 >/dev/null 2>&1
    elif command -v dnf >/dev/null 2>&1; then
        dnf install -y curl wget ca-certificates unzip iproute python3 >/dev/null 2>&1
    elif command -v yum >/dev/null 2>&1; then
        yum install -y curl wget ca-certificates unzip iproute python3 >/dev/null 2>&1
    elif command -v apk >/dev/null 2>&1; then
        apk add --no-cache curl wget ca-certificates unzip iproute2 python3 >/dev/null 2>&1
    else
        echo "无法识别系统包管理器"
        return 1
    fi
...
try:
    ...
    if security != "tls":
        raise Exception("此选项要求 security=tls")

    if transport_type != "ws":
        raise Exception("此选项要求 type=ws")
    ...
    # 如果当前通过 SSH 连接，尽量避免把当前 SSH 客户端 IP
    # 也强制套进 TUN，降低断线风险。
    import os
    ...
    if ssh_client:
        ...
        if ":" in client_ip:
            exclude.append(client_ip + "/128")
        else:
            exclude.append(client_ip + "/32")
...
try:
    ...
    if security != "reality":
        raise Exception("此选项要求 security=reality")
    ...
    if not sni:
        raise Exception("缺少 sni")

    if not pbk:
        raise Exception("缺少 pbk")
    ...
    if ssh_client:
        ...
        if ":" in client_ip:
            exclude.append(client_client_ip + "/128")
        else:
            exclude.append(client_ip + "/32")
...
apply_config() {
    ...
    if systemctl is-active --quiet sing-box; then
        echo
        echo "全局代理已启动"
        echo "VPS 出站流量现在通过 sing-box TUN 转发"
    else
        echo
        echo "sing-box 启动失败"
        echo
